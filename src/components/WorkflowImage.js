...
const WorkflowImage = ({ setIsLoading }) => {
    ...
    const handleAnalyze = async () => {
        ...
        if(res.status){ setAnalysis(res.result) } 
    ...
    const handleGenerate = async () => {
        ...
        if(res.status) { setImage(res.result); }
        else { setError(res.error) }
        setIsLoading(false);
    };
                ...
                <div className="flex flex-col items-center justify-center w-full">
                    <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-white hover:bg-gray-50 transition-colors">
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p className="mb-2 text-sm text-gray-500 font-medium">Click to upload reference image</p>
                        </div>
                    ...
                    {file && (
                        <div className="mt-5 flex flex-col items-center">
                            ...
                            <button className="flex flex-row gap-2 px-5 py-2 rounded-lg bg-indigo-600 text-white shadow-sm hover:bg-indigo-700 transition-all" onClick={handleAnalyze}>
                                <Search size={20} />
                ...
                <div className="mt-5 border-t pt-8">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                        ...
                        <div className="flex flex-col">
                            ...
                            />
                            <button className="flex flex-row gap-2 w-full px-6 py-2.5 rounded-lg bg-blue-500 text-white justify-center shadow-md hover:bg-blue-600 transition-all" onClick={handleGenerate}>
                                <Wand2 size={20} />
                                <span>Generate Variation</span>
                            </button>
                            <button onClick={() => {setAnalysis(""); setFile(null); setImage("")}} className="mt-4 text-xs text-gray-400 hover:text-gray-600 underline">Upload a different image</button>
                        </div>
